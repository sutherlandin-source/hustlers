...
export async function listTransactions(req, res, next) {
  try {
    ...
    const transactions = await escrowService.listTransactions(req.user.userId, filter, options);
...
export async function getTransaction(req, res, next) {
  try {
    const transaction = await escrowService.getTransactionById(req.params.id, req.user.userId);
